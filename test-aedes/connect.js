...
test('second CONNECT Packet sent from a Client as a protocol violation and disconnect the Client', function (t) {
  ...
  s.broker.on('clientReady', function () {
    ...
    setImmediate(() => {
      ...
      t.ok(
        s.conn.destroyed,
        'close connection if packet is a CONNECT after network is established'
      )
    })
  })
})
...
test('connect handler calls done when preConnect throws error', function (t) {
  ...
  const { handleConnect } = require('aedes/lib/handlers/connect')
...
test('handler calls done when disconnect or unknown packet cmd is received', function (t) {
  ...
  const { handle } = require('aedes/lib/handlers/index')
...
test("After first CONNECT Packet, others are queued until 'connect' event", function (t) {
  ...
  broker.on('client', function (client) {
    t.equal(
      client._parser._queue.length,
      queueLimit,
      'Packets have been queued'
    )
...
;[
  ['fail with no error msg', 3, null, false],
  ['succeed with no error msg', 9, null, true],
  ['fail with error msg', 6, new Error('connection banned'), false],
  ['succeed with error msg', 6, new Error('connection banned'), true],
